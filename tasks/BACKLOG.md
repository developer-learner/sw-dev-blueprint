...
**Gate-flip acceptance criteria (four negative cases — the flip is BLOCKED until all four are green in the supported Linux setup with real fixture keys).** A successful signed run proves the happy path only; each of these is a confirmed bypass in the current verifier/orchestrator (read against `979076c`, 2026-09-04) that the flip would otherwise turn *on* without closing. Each criterion is a behavioral test the flipped `--gate` (and orchestrate) must satisfy. **Implementation status (2026-09-05):** ALL FOUR implemented + selftested on branch stack `prov-c1-unsigned-rejection` → `prov-c3-evidence-by-role` → `prov-c4-finalization-surfacing` → `prov-c2-durable-activation` (full control-plane suite 598 passed locally); brief at [`tasks/m2b-provenance-fixes-brief.md`](m2b-provenance-fixes-brief.md). Pending push + CI (ruff/mypy/coverage) round-trip, then the live child adoption run and the T1 flip.

1. **[DONE — C1]** **Unsigned commit is rejected once provenance is active.** Fixture: a signed broker commit (establishes the boundary), then an in-scope task commit carrying valid `Swbp-Role`/`Swbp-Model`/`Swbp-Run` trailers but *no* signature. `--gate` must exit nonzero and name the missing signature. *(Fixed: [check-provenance.py:249](../scripts/check-provenance.py:249) now appends an unsigned-commit failure after the activation boundary.)*

2. **[DONE — C2]** **Activation boundary is durable, not window-derived.** The in-scope boundary must not be recomputable away by narrowing the inspected range. *(Fixed: the verifier reads a durable anchor from `scripts/.provenance/activation` in the inspected tip's tree and scopes each commit by strict-descendant ancestry against it; a narrow window still gates descendants; a live-but-unanchored history and an unreachable anchor both fail explicit. New broker `activate [<commit>]` subcommand records the anchor, monotonic.)* Remaining flip-time piece (tracked with the T1 flip, not the criterion): wire the gate to the actual push/PR range rather than the dev-default `HEAD~50..HEAD`.

3. **[DONE — C3]** **Evidence is required by role, not optional.** An in-scope model-generated commit (`Swbp-Role` = coder|em, `Swbp-Run` ≠ n/a) with BOTH evidence-hash trailers removed must FAIL `--gate`. Human/tpm and bookkeeping (`pipeline`) commits are the explicit exemptions and must still pass. Broker side: when a model role declares a real capture, capture must fail closed if prompt/reply files are absent rather than silently skipping. *(Fixed: verifier role-gate at [check-provenance.py:285](../scripts/check-provenance.py:285) + `REQUIRED_EVIDENCE_ROLES`/`EVIDENCE_SCHEMA`; broker fail-closed + `Swbp-Evidence-Schema` at [git-provenance.sh:182](../scripts/git-provenance.sh:182).)* Also closed the `Swbp-Run: n/a` model-commit escape.

4. **[DONE — C4]** **Finalization failure surfaces; the run does not exit 0.** Fixture: force the `[success]` commit to fail at its site (e.g. a signing failure). Orchestrate must persist durable results (completion ledger, measurement, evidence), then report a distinct "validation passed; finalization failed" and exit NONZERO. *(Fixed: `finalize_success` in [orchestrate.sh](../scripts/orchestrate.sh) reorders persist → commit → teardown and exits 3 with the checkpoint kept on a failed [success] commit.)*

**Environment precondition:** the provenance suite was NOT green on the operator's macOS host on 2026-09-04 (12 fail / 3 pass, fixture-key generation unavailable). Re-verified 2026-09-05: fixture-key generation now works and the full control-plane suite is 593 passed on the macOS host — the earlier blocker was transient/environmental. A clean CI (Linux) run still gates the "verified" claim (ruff/mypy/coverage live only in CI).
