...
- [ ] [B] **T7 (backlog #13) — model-specific Git provenance: M1 DONE (D-174), M2 gated.** CEO approved Option 3 as staged, M1 authorized initially. M1 landed: trusted commit broker (`scripts/git-provenance.sh`), author/committer separation, `Swbp-*` trailers, provider-returned model preference (llm-call meta sidecar), run-id, coder-prompt capture, 10 blind selftests (suite 548 green). M2 (GPG signing + verifier) authorized only after: (1) durable trust anchor — pinned fingerprint + rotation/revocation, out-of-band of the verified repo; (2) durable evidence storage for prompt/reply bytes; (3) attestation-semantics wording (signing attests what the pipeline recorded, not which model generated). Vortex/Testchat adoption = their own `update-template.sh` pull (their LLMs).
- [ ] [B] **T11 (backlog #18) — mature OSS adoption subject #2: Rich approved for Phase 0.** CEO approved Rich (15.0.0, MIT) as subject #2. Phase 0 executing: clone pinned at tag `v15.0.0` (commit `6ac483c`), plane install at the post-M1 Blueprint SHA, legacy-suite byte-hash pin, pre-spec tunnel state recorded, clean baseline. Phase 1 (first freeze + milestone M–L) schedules around the single-run machine constraint — NOT started.
