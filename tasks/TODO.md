...
## 4. Born-linked pivot
> PARKED 2026-08-25 (CEO) → **CLOSED 2026-09-03**: CEO scope call made ("do 2"), both items below done.

- [x] [B] Scope first, then build — seed path yields project-local files + links, skipping the copy. Precision: one template-derived file stays real (`check-drift.yml`, GitHub necessity); `ci.yml`/`container-build.yml` are project-local by design — **done 2026-09-03 as D-183 (`a87f014`)**: `new-project.sh --linked` seeds child-owned files + a transient copy-seeded plane skeleton, then converts to the linked state in the same run (seed → `[template-link]` → bootstrap); the check-drift exception and project-local CI are exactly as scoped; `BLUEPRINT.md`/`QUICKSTART.md` joined the plane manifest; selftest `test_born_linked_seed` + real smoke child verified (82/82 plane paths, gate green, blueprint untouched)
- [x] [V] Verify vortex integrity after seed-path changes (manifest, symlinks, full suite) — **done 2026-09-03**: vortex re-pinned `24fcd94 [template-link 20902780b68d]`; control-plane suite 564 passed (release-gate), full product suite 121 passed, CI + check-drift green; root docs now manifest-governed symlinks
...
- [x] [B] **T6 (backlog #9) — observe one organic two-strike ladder climb.** CEO: leave untouched until an organic qualifying run occurs. Status 2026-09-01: no new organic run since 2026-08-30 (last: spec v20 success, plane `605a998`). Rungs observed so far span multiple runs — 08-22/23: schema-valid `brief_wrong` diagnosis + materially revised brief + usable TPM batch; 08-28: D-71 diagnosis retry rung (2× schema-invalid → retry) then a D-169 `transient_or_environmental` halt; 08-28→08-30: task-level failure, resume, success. Still missing per the backlog's own close criterion: **one uninterrupted organic run** exercising the complete retry → diagnosis → revised-brief/TPM path with the outcome accepted against D-70/D-69. Observe, don't manufacture — closes on the next qualifying run. **CLOSED 2026-09-03 (CEO call: validated):** the v115 run (testchat T8 build, 2026-09-02, plane `6132185`) was the uninterrupted qualifying run — T1/T3 each struck twice (8 coder calls) → schema-valid `brief_wrong` diagnoses → in-run revised briefs (prompt-diff proven) → caps-exhausted TPM bundles → batch halt → v116 brief-only refreeze (`641aa8d`) → v119 success (`aa3deea`), D-69 budget contained. Evidence: blueprint BACKLOG Completed row (2026-09-03).

## 8. Delegated serialized items (Track C session, 2026-09-01)

- [x] [T] **T8 (Testchat) — router recut onto Vortex v26's full ready set: v115 spec DRAFTED + staged, build awaiting the machine window.** Offline spec authoring complete (testchat `ed9ffe0`): AC-175..AC-181 supersede the v107 single-model seams (AC-170..173) — full dynamic ready set from `GET /v1/models`, any-ready-model routing to `{VORTEX_URL}/v1/chat/completions`, not-ready fall-through to the local path, the mid-flight 404-race not-ready message (exact string pinned), generic fallback while still ready, `ROUTER_MODEL_ID` retired. 20-test oracle staged; all refreeze preflights green on `--diff`; vs unmodified code 8 failed / 12 passed (the 8 = new-behavior pins). Durable copy in testchat `tasks/T8-v115-spec-draft/`. **Remaining (schedule-gated, not started):** freeze + build in the **Linux dev VM** around the one-live-run-at-a-time slot (fires the local coder; needs a memory/RAM window) — `refreeze.sh scripts/.approved/incoming` (D-121 auto-apply) then `orchestrate.sh`. Precedes T9 (deployment cutover). **DONE 2026-09-02:** freeze + build ran in the VM window — v115 built (the run that also closed T6: two organic two-strike climbs), v116 brief-only refreeze `641aa8d`, **v119 success `aa3deea`** (8-task run, no-edit). T9 (deployment cutover) is the successor, tracked in testchat's own backlog.
...
- [ ] [V] Post-blueprint-landing verification — full product suite (all test files, scope labeled, never a subset) + CI green, after every blueprint commit touching linked files
  - **Executed 2026-09-03 (pi session, D-183 landing `a87f014`/`2090278`):** the commits touched linked plane files (`new-project.sh`, `bootstrap.sh`, `selftest_linked_template.py`, `.manifest-template`, `QUICKSTART.md`) — verification owed and done. Vortex re-pinned `24fcd94 [template-link 20902780b68d]`: control-plane suite 564 passed (release-gate), **full product suite 121 passed (all test files, scope: full suite)**, CI + check-drift green. Testchat re-pinned `9d6195f` (on the other lane's `7311949`): control-plane 564 passed, CI + check-drift green (full product suite via CI). Both root docs now manifest-governed symlinks in both children.
