...
- **Prefer mechanical gates over doc guards** for must-hold rules. The memory layer is best-effort, not enforced (see correction log row 6). For line-count, step-number reachability, schema validity, or legacy-tool-residue checks, the enforcement is `scripts/check-doc-sanity.sh` — runs in CI on every push, fails the build on regression. Doc guards (this section, BLUEPRINT.md) are strong hints to a session, not hard gates. The CI script is the hard gate.
...
| 2026-06-04 | Built `scripts/check-doc-sanity.sh` to upgrade the line-count, step-reachability, and schema-validity guards from advisory (doc-only) to enforced (CI gate). Runs in GitHub Actions on every push to main; fails the build on regression. Also catches: legacy CLI tool residue, broken `AGENTS.md` symlink, invalid `opencode.json` JSON. The script excludes itself from the legacy-tool residue check (its own grep string would otherwise trip on its own code) — that self-exclusion is documented in a comment so a future reader doesn't strip it. | When graduating a doc guard from advisory to enforced, the bar is: (1) the rule is verifiable by a one-line script check, AND (2) the failure mode is bad enough that you want CI to fail rather than just log. The current 5 checks meet both. Adding new checks: keep the script self-contained, no external deps beyond `grep` / `wc` / `jq`; print actionable error messages; exit non-zero on failure. The doc guard in the log above stays as the *why*; the script is the *what*. |
