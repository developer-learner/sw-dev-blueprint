...
| A **~27B-class dense local model, non-thinking, 32K context, 8-bit or higher quantization** | The proven floor for the coder/EM seats — smaller or heavily-MoE models failed task-level work in this repo's own history (D-12/D-14/D-66); 4-bit is acceptable only for a maintenance-scoped project after a per-project bench proves parity (D-72), never as a new-project default | "Non-thinking" is a hard rule (BLUEPRINT.md Rule 1) |
