...
## D-72 — 2026-07-17 — Quantization tier for EM/coder seats: 8-bit preferred; 4-bit acceptable only for maintenance-scoped work

**Decision:** For BOTH EM and coder seats, prefer **8-bit** (or higher) quantization on the local model. **4-bit** is acceptable ONLY when the project is in maintenance mode (small, well-scoped edits, tight briefs, short context) AND has a demonstrated per-project bench proving parity on that project's authoring shape. Never adopt 4-bit as a default for a new project — the seat sits in the highest-transcription-precision role in the ladder (D-66), and 4-bit measurably degrades exactly the axes the seat is graded on: verbatim copying, numeric precision, long-context faithfulness. The CEO's operational choice sits in `models.env`; this decision is guidance the operator applies at role-mapping time, not a mechanical gate — the blueprint has never gated by quantization identity, per D-41.

**Reason:** Testchat's M25 web-search milestone ran with `ddalcu/Qwen3.6-27B-4bit-MTP-MLX-Serve` in both seats (CEO-directed trial). Empirically excellent on that milestone: coder character-perfect across 7 files, EM plan first-try valid, D-71 diagnosis schema-valid on the first live-fire. Head-to-head benchmark against `mtplx-qwen36-27b-optimized-quality` (8-bit) on identical prompts: 4-bit 1.4×-1.7× faster on realistic pipeline shapes (prefill 726 t/s at 1489-token contexts, decode 54-92 t/s vs 8-bit 36-53 t/s wall-effective). But the sample is one well-scoped maintenance-shape milestone. The known 4-bit failure modes (perplexity climb past ~16K context, drift on exact wording — precisely the D-66 transcription-precision axis, weaker multi-step reasoning under state pressure) did not surface here because the ERD was tight, briefs were short, and context stayed under 4K per call. They would surface on: new-feature milestones with 4+ files, ERDs that stress the EM's exact-copy discipline, briefs approaching the 2500-char cap, or diagnosis paths that need real reasoning rather than schema echo. The 8-bit seat carries known behavior across every prior testchat milestone; 4-bit carries one.

**Do not suggest:** starting a NEW project on 4-bit "because it's fast" — the seat quality matters most at the outset, when the ERD authoring conventions and the coder's edit-block fluency are being established with the operator; performance-tune later once you have a working baseline; treating a passing selftest suite as license to run 4-bit on complex work (Rule 6: selftests are structural coverage, not seat-quality coverage — the seat's grading happens in live pipeline runs, milestone-by-milestone); running a 4-bit seat on complex work WITHOUT the 8-bit variant loaded as a switchable backup (the swap is one env-var change; there is no reason not to keep the safety on).
