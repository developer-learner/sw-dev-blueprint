...
**Reason:** Principles handed to a tier are aspirational; the gates (`phase-gate` INV-2, `validate-plan`, refreeze gates, `check-swallowed-errors` D-68) are load-bearing, and live CEO acceptance (D-44) is the completion check. The value is (1) closing the dangling-reference gap so the coder actually receives its conventions, and (2) giving the cold reviewer a decision-priority-ordered checklist for the "worst plausible failure" question it already asks. EM/TPM delivery deferred; the promotion trigger to fleet-wide is a child review seat.

**Do not suggest:** pointing the conductor at the rubric (it is a dispatcher/reporter, D-04 class — widening its lane is the exact failure the 2026-07-04 correction logs); a blanket `print()`/`T20` gate (false-positives on CLI output); fleet-distributing the rubric before a child review seat exists; adding a canonical-parent projection mechanism for two small hand-maintained docs before they drift.
...
**Reason:** Vortex's first milestone ran while the blueprint advanced six commits mid-flight; hash manifests made the resulting pin-drift loud but still cost a failed launch cycle, and nothing would have stopped a *partial* mixed-version run had the drift landed between two gates instead of at one. Symlink execution makes "which code am I running?" unanswerable mid-run; content-addressed snapshots make it a constant. Testchat's first adoption proof then exposed that the original implementation only supported Vortex's symlink experiment: an ordinary copied child cannot have the Blueprint commit in its own Git object database. The manifest-verified installed-plane path preserves offline child operation without weakening byte integrity.
...
## D-166 — 2026-08-16 — Active ERD context deduplicates execution payload without dropping skipped-freeze instructions

**Decision:** `validate-plan.py --active-erd-context` renders a semantic compact view when an unfinished milestone spans multiple freezes. The immutable `ERD-DELTA-vN.md` files and plan synthesis still consume the complete ordered D-140 range. Only the model-facing view changes: the newest freeze's narrative sections are kept; earlier changed and superseded AC sections remain versioned; historical preambles, changed-file restatements, and assumptions are audit history already superseded by the current capsule and exact active JSON inventory; identical verbatim coder briefs, test-to-file pins, and resolved DAG edges are emitted once in first-seen order; every unique incremental execution brief remains. A single-freeze milestone is emitted unchanged. The renderer falls back to the raw ordered packet if its scaffolding would expand the source. `orchestrate.sh` measures the result against a new 32,768-byte `active-erd-context` warning budget before assembling the existing 65,536-byte complete EM package.

**Alternatives considered:** (a) Emit only the newest ERD delta, as testchat temporarily did — rejected for the template: D-140 exists because a later freeze can omit still-unexecuted work from an earlier skipped freeze; latest-only is safe only after a mechanically enforced cumulative-capsule protocol exists. (b) Delete coder briefs from the delta — rejected: their verbatim presence enables D-133 mechanical synthesis and avoids a 45–90 minute EM transcription call. (c) Keep concatenating raw capsules — rejected: testchat v107–v113 produced roughly 82.5 KB of ERD text and a measured 96.8 KB diagnosis call against the 65.5 KB package budget; the same mapping, DAG, and unchanged briefs appeared repeatedly. (d) Hard-fail the new component budget — rejected under D-145: a genuinely large but complete active milestone must remain runnable, and the complete-package warning remains the final regression signal.

**Reason:** The ERD delta itself was not the primary bloat: testchat's current capsule was 9.4 KB. The growth came from treating seven immutable, near-identical snapshots as seven independent model instructions. On the real v107–v113 artifacts, the compact renderer reduces the ERD packet from about 82.5 KB to 27.9 KB while preserving every earlier AC change and every unique brief, mapping, and DAG edge. This removes repetition and superseded narrative without reinstating the exact newest-file data-loss class D-140 closed.

**Do not suggest:** switching the template to unguarded latest-only context; deleting immutable ERD snapshots; deduplicating unique incremental briefs merely because they target the same file; moving plan synthesis onto the compact model view (synthesis consumes raw authority); turning the 32,768-byte component warning into a hard halt without a separate policy decision.
...
> **Amended by D-166 (2026-08-16):** the ordered raw instruction range remains
> authoritative for plan synthesis and recovery; model-facing calls receive a
> semantically equivalent view with repeated briefs, mappings, and DAG edges
> deduplicated. This does not permit newest-delta-only context.
...
**Amendment (2026-09-03, CEO-directed): the gate is diff-scoped, not whole-file.** `scripts/lint-changed.py` runs the SAME `ruff check` (same rule set / child config resolution — no `--select`) but reports only findings on lines the task changed relative to its strike-0 baseline (HEAD before the first attempt, captured once and reused across strikes so a later strike cannot pass off an earlier strike's new violation as legacy). A pre-existing finding on a line the coder was never briefed to touch — and under D-59 anchored edits cannot touch — no longer burns a strike; a created file diffs as wholly added, so it is linted in full. Syntax errors (`E999`/`invalid-syntax`) and I/O errors (`E902`) gate regardless of line scope; a genuine ruff tooling error (rc≥2) still dies fail-closed; an uncomputable baseline falls back to whole-file (fail-strict). Proven by `scripts/selftest/selftest_lint_changed.py`. This was a stop-and-ask class change (it relaxes what the gate flags) taken on explicit CEO direction; the whole-file behavior had made a correct anchored edit un-shippable whenever the edited file carried unrelated legacy debt.
...
**Do not suggest:** widening the gate to files the task did not write (INV-2 owns the lane; lint debt elsewhere is not this task's evidence); reverting to whole-file scope to "catch legacy debt too" (the coder cannot fix a violation outside its D-59 anchored edit, so whole-file lint made a correct edit un-shippable — pre-existing debt in an edited file is a TPM/refreeze concern, never a coder strike); demoting the halt-on-missing-ruff to a skip ("the gate ran zero times" and "the gate found zero issues" must stay distinguishable, Rule 6); demoting the syntax/IO-error or rc≥2 always-gate to scope-filtered (a file that will not parse or read cannot be graded); bolting formatting (`ruff format`) onto the gate (style churn in a retry loop burns strikes on non-defects; the check gate flags real findings only).
...
## D-35 — 2026-07-01 / triggered 2026-08-23 — Fleet Tier 3: children link one versioned Blueprint control plane

**Status:** The deferred trigger fired on 2026-08-23. Vortex's partial symlink experiment and Testchat's copied plane had already diverged in distribution shape, and a child-portability regression repeatedly passed in one shape while failing in the other. The CEO explicitly chose one Blueprint-backed plane for both children.

**Decision:** A linked child records `mode=linked` and a relative Blueprint checkout in `.template-link`. Every path in the Blueprint commit's `.manifest-template` is a relative symlink to that checkout except `.github/workflows/check-drift.yml`, which remains a physical bootstrap file because GitHub must parse workflow YAML before any checkout step can reconstruct the dependency. `.template-version ref=<sha>` and `.manifest-template` pin the adopted version and bytes; `phase-gate.sh` rejects a missing source, wrong target, local same-byte copy, or hash mismatch. `scripts/link-template.sh` performs copy-to-link migration and later adoption atomically; ordinary `update-template.sh` delegates to it in linked mode so an update can never write through a symlink into Blueprint. Interactive runs see one shared checkout, while D-168 still re-executes each milestone from an immutable snapshot of the pinned plane. Release verification and GitHub CI materialize the exact pinned Blueprint beside isolated child checkouts before testing.

**Alternatives considered:** (a) Preserve copied distribution — rejected after the same Blueprint regression behaved differently in Vortex and Testchat; each child still owned dozens of executable duplicates. (b) Keep Vortex's hybrid absolute-symlink POC — rejected: ten copied template files remained divergent, links were host-specific, and the ordinary updater could write through them. (c) Publish a package/archive immediately — clean provenance but adds registry and extraction machinery without improving the two local active children; the Git commit plus manifest is already the versioned artifact. (d) git subtree/submodule — subtree pollutes child history; submodule UX and recursive checkout failure modes add no integrity beyond the pinned linked checkout.

**Reason:** The trigger was operational, not fleet-count vanity: maintaining two distribution forms created real portability defects and made “the control plane is green” depend on which child ran it. One linked, commit-pinned source removes duplicated executable ownership while retaining offline, immutable milestone execution and a physical CI bootstrap.

**Do not suggest:** Restoring copied template files to fix a child locally; fix Blueprint and adopt a new pin. Replacing the physical drift workflow with a symlink (GitHub cannot bootstrap it). Pointing `.template-link` at a moving or unverified directory; the structural and byte gates intentionally fail closed.

---

## D-34 — 2026-07-01 — Template propagation: update-template.sh (the refreeze pattern, applied to the control plane)

> Amended by D-96 (auto-apply default), D-101 (upstream removals), and triggered D-35 linked distribution (2026-08-23).

**Decision:** Copied children pull control-plane improvements with `scripts/update-template.sh`: it resolves the template (a local clone via `--from`, or `gh repo clone` of the `.template-version` slug), takes the file list from the **template's** `.manifest-template` at the target ref (so additions and removals flow in), shows one aggregate diff, applies contents, exec bits, and removals, installs the template's manifest verbatim, advances `ref=` in `.template-version`, regenerates `.manifest-project`, runs the integrity gate as a post-apply check, and commits `[template-update <sha>]`. In D-35 linked mode the same command delegates to `scripts/link-template.sh`, which changes the link set and physical workflow without writing through links. `--dry-run`, `--review`, hash-bound `--approve`, and opt-in `--interactive` remain available under D-96. `--stamp` mode retrofits a pre-D-33 child.
